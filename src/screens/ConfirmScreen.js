...
import { createData, getUserDetailsByPhone, registerApi, updateData } from '../context/Api';
let obj;
...
const PageContent = ({navigation,route}) =>{
    const {appState:{saveUser,notificationToken,fontFamilyObj:{fontBold,fontLight},showToast} } = useContext(AppContext);
    ...
    const confirm = () =>{
        if(confirmationCode.toString() === obj.code.toString()){
            ...
            obj = {...obj,id,deviceid:notificationToken}

            registerApi(obj,(response) => {
                if(response.success){
                    saveUser({...obj,firstname:obj.fname});

...
const ModalSelector = memo((props) => {
    const {item,handleFilters} = props.attr;
    const {appState:{
    ...
    return (
        <View style={{padding:10}}>
            {item?.subItems?.map(({type},i) => 
                <TouchableOpacity onPress={() => {
                    setModalState({isVisible:false});
                    handleFilters(item.type,type);
                    ...
                    <Text style={{fontFamily:fontLight,marginLeft:10,marginTop:5}}>{type}</Text>
                </TouchableOpacity>
            )}
        </View>
    )
